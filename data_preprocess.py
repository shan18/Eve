...
def map_line_to_id(lines):
    """ Map each line to its id in a dictionary """
    id2line = {}
    for line in lines:
        _line = line.split(' +++$+++ ')
        if len(_line) == 5:
...
def create_conversation_list(conversations):
...
def separate_questions_answers(id2line, conversations_ids):
    """ Separate questions and answers from the dataset """
    questions, answers = [], []
...
def clean_text(text):
    """ Simplify the text to make the training process easier """
    text = text.lower()
...
def clean_questions_answers(questions, answers):
    # clean the questions
    questions_clean = []
...
def map_occurrences(questions_clean, answers_clean):
...
def map_data_with_threshold(word2count):
    ...
    for word, count in word2count.items():
        if count >= threshold:
            ...
            word_number += 1
    return questions_words2int, answers_words2int


def add_last_tokens(questions_words2int, answers_words2int, answers_clean):
    ...
    return questions_words2int, answers_words2int, answers_clean


def translate_clean_data(questions_clean, questions_words2int, answers_clean, answers_words2int):
    """ Translate all questions and answers to integers and replace filtered out words by <OUT> """
    questions_to_int = []
    for question in questions_clean:
        clean_question = []
        for word in question.split():
...
def sort_translated_data(questions_to_int, answers_to_int):
...
def preprocess_input():
    """ Preprocess the input data
        ...
        Step 2: Map each line to a unique id
        Step 3: Separate questions and answers from the dataset
        Step 4: Simplify the text in the dataset
        Step 5: Remove less frequent words based on a threshold
        Step 6: Map the questions words and answer words to an unique integer
    ...
    questions_clean, answers_clean = clean_questions_answers(questions, answers)
    word2count = map_occurrences(questions_clean, answers_clean)
    questions_words2int, answers_words2int = map_data_with_threshold(word2count)
    ...
    questions_to_int, answers_to_int = translate_clean_data(
